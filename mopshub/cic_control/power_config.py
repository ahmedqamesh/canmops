        ...
        try:
            if bool(self.locked_by_sys[__channel]) is False:
                if bool(self.locked_by_user[__channel]) is False or set_flag is not None:
                    ...
                    self.logger.info(f'Write to Latch address: {channel} , value = {data} was successful')
                    return data
                else:
                    logging.error(f'Power of Channel {__channel} was locked by user')
            else:
                logging.error(f'Power of Channel {__channel} was locked by sys while start up')
        except Exception as e:
            self.logger.exception(e)
            self.logger.error(f'Error while writing to Latch address = {channel}, value = {data}')
            return None
