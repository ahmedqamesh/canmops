...
class CICreadout(MPconfig):
    ...
    def __init__(self):
        ...
        self.channel_value = ("UH", "UL", "VCAN-MOPS", "Temperature", "Current1", "Current2", "Next", "Next")
    ...
    def read_adc(self, device, mp_channel, can_interface):
        ...
        msg_out = [self.__address_byte[0], 0,
                   ...
                   self.__address_byte[6], 0,
                   self.__address_byte[7], 0,
                   0, 0]
        try:
...
readout = CICreadout()
for i in range (5):
    adc = readout.read_adc(0, 31, 1)
    #print(f"{adc[0]}    {adc[1]}")
    print(f"{adc[0]}    {adc[1]}    {adc[2]}    {adc[3]}    {adc[4]}    {adc[5]}    {adc[6]}    {adc[7]}")
    time.sleep(2)
